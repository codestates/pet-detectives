...
module.exports = {
  petregisterController: async (req, res) => {
    ...
    if (
      ...
      !req.body.pet_name ||
      !req.body.pet_sex ||
      !req.body.pet_category ||
      !req.body.pet_lost_region ||
      !req.body.pet_lost_date ||
      !req.body.pet_age
    ) {
      return res.status(404).send({ message: "펫 정보를 모두 입력해주세요" });
    }
  ...
  petdeleteController: async (req, res) => {
    ...
    const deletePet = await user.findOne({
      where: { email: accessTokenData.email },
    });
    console.log(deletePet);

    const delPet = await post
    ...
    if (delPet === undefined) {
      return res
        .status(400)
        .send({ message: "펫이 삭제되었거나 존재하지 않는다" });
    }

    post
      .destroy({ where: { id: req.body.id, user_id: deletePet.id } })
      .then((data) => {
        console.log(data);
  ...
  peteditController: async (req, res) => {
    ...
    const editPet = await user.findOne({
      where: { email: accessTokenData.email },
    });
