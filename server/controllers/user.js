...
const {generateAccessToken,authorized,sendRefreshToken,generateRefreshToken, sendAccessToken} = require('../middleware/tokenfunction');
...
module.exports = {
  userinfoController: async (req, res) => {
   ...
   const token =  req.headers.token //!헤더로 토큰 받은경우
    ...
    if(!accessTokenData){
      return res.status(401).send({messagea:'인증정보가 올바르지 않습니다.'})
    }

    user.findOne({where:{email:accessTokenData.email}}).then(user =>{
      

      post.findAll({where:{user_id:user.dataValues.id},
        Attributes:['pet_name','pet_lost_date','pet_lost_region','description','pet_age',]}).then(pet =>{

        return res.status(200).send({user:user.dataValues,pet:pet.dataValues})
      })
...
user.update({ nickname:newNickName},{where: {email:accessTokenData.email}
    }).then(data =>{

  return res.status(200).send({data:data,message:'닉네임이 변경되었습니다.'})
}).catch((err)=>{
  console.log(err)
})
 ...
 //새비밀번호와 현재 비밀번호가 존재 ,
user.update({ password:newPassword},{where: {email:accessTokenData.email}
    }).then(data =>{

  return res.status(200).send({data:data,messagea:'비밀번호가 변경되었습니다.'})
}).catch((err)=>{
  console.log(err)
})
...
//정보가 없다면 삭제 x
    if(!accessTokenData){
      return res.status(403).send({message:'존재하지 않는 유저 이거나, 이미 탈퇴된 유저 입니다.'})
    }
    //유저정보삭제
    user.destroy({where:{email:accessTokenData.email}}).then(data =>{
      ...
      res.cookie('refresh',cookie,{httpOnly:true,sameSite:'none',maxAge:1})
